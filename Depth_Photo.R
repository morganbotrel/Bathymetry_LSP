##CALCUL DES PROFONDEURS LORS DE LA PRISE DE PHOTOGRAPHIES AÉRIENNES##
...
#Ouvrir les données des photos aériennes
photo <- read.table("./Raw_data/Photo_date_heure.csv", header=TRUE, sep=";")
#Convertir les dates pour avoir le même format que le fichier d'échosondage
photo <- data.frame(
  Date=as.Date(photo$Date),
  Time=format(as.POSIXct(photo$UTC_start,tz="GMT",format="%H:%M"), format="%H:00") #Utiliser le début
)
...
#Déterminer le niveau d'eau lors des prises de photo
...
eA2015_SLEV$Zphoto <- eA2015_SLEV$Zecho +  (photo_SLEV[3,3]- eA2015_SLEV$SLEV)
